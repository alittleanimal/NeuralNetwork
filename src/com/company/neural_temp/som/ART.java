...
public class ART extends Training implements Validation {
    ...
    private int calcWinnerNeuron(NeuralNet n, int row_i, double[][] pattern) {
        ArrayList<Double> listOfEstimatedOutput = new ArrayList<>();
        ...
        for (int i = 0; i < SIZE_OF_OUTPUT_LAYER; i++) {
            ...
            for (int j = 0; j < SIZE_OF_INPUT_LAYER; j++) {
                ...
                netValue = netValue + (weightIn * pattern[row_i][j]);
                neuron_i++;
            }
        ...
        return listOfEstimatedOutput.indexOf(Collections.max(listOfEstimatedOutput));
    }
    ...
    private boolean vigilanceTest(NeuralNet n, int row_i) {
        ...
        return vigilanceValue > n.getMatchRate();
    }
    ...
    public double[][] netValidation(NeuralNet n) {
        ...
        System.out.println(Arrays.deepToString(matrixEstimated));
